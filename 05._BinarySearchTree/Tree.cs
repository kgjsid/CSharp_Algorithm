...
namespace _05._BinarySearchTree
{
    public class Tree
    {
        ...
        //     상위스킬을 배워야 하위스킬을 배울 수 있는 스킬트리, 몬스터의 행동트리
        ...
        {
            public T item;                      // 노드가 가지고 있는 값

            public TreeNode<T> parent;          // 부모 노드
            public List<TreeNode<T>> children;  // 자식 노드들
        }

        // 모든 트리의 관리?
        // -> 모든 트리는 이진 트리로 관리가 가능함

        //      o        |          o
        // o o o o o o   |     o         o          => 두개 다 동일한 구조
        //               |  o     o   o     o
        ...
        // 트리의 순회?
        // 처음부터 끝까지 일자 형태가 아님(비선형)
        // -> 순차적으로 처리하고 싶다면 어떤 순서인지 규칙을 정해야 함(트리 순회)
        

        // <이진트리 순회>
        // 트리는 비선형 자료구조로 순차적 처리를 위해 순서에 대해 규칙을 선정해야 함
        // 순회의 방식은 크게 전위, 중위, 후위 순회가 있음
        // 전위순회(Pre-Order)  : 노드, 왼쪽, 오른쪽(위에서 먼저 찾을 경우 유용)
        // 중위순회(In-Order)   : 왼쪽, 노드, 오른쪽(이진탐색트리의 경우 정렬을 보장)
        // 후위순회(Post-Order) : 왼쪽, 오른쪽, 노드(폴더 삭제 / 수식계산 시 유용)
        //
        //        A
        //    ┌───┴───┐
        //    B       C
        ...
        // 중위순회 : 부모를 기준으로 왼쪽 오른쪽으로(왼쪽 / 노드 / 오른쪽)
        // 1. (         ) A (         )
        // 2. (( ) B ( )) A (         )
        // 3. ((D) B (E)) A (         )
        // 4. ((D) B (E)) A (( ) C ( ))
